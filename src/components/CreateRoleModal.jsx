...
const CreateRoleModal = ({ show, onClose, onRoleCreated }) => {
  ...
  const handlePermissionChange = (dept, action) => {
    // Enforce READ pre-requisite for UPDATE and DELETE
    if (
      (action === "UPDATE" || action === "DELETE") &&
      !permissionsMatrix[dept]?.READ
    ) {
      alert(`⚠️ '${action}' permission requires 'READ' permission to be enabled first.`);
      return;
    }
  ...
  const handleCreateRole = async (e) => {
    ...
    // Validate READ pre-requisite again before submission
    for (const [dept, perms] of Object.entries(permissionsMatrix)) {
      if ((perms.UPDATE || perms.DELETE) && !perms.READ) {
        setError(`❌ Department '${dept}' has UPDATE/DELETE but missing required READ permission.`);
        return;
      }
    }
    ...
    Object.entries(permissionsMatrix).forEach(([dept, perms]) => {
      Object.entries(perms).forEach(([action, isChecked]) => {
        if (isChecked) {
          permissionSet.add(`${dept.toUpperCase().replace(/\s/g, "_")}_${action}`);
        }
      });
    });
